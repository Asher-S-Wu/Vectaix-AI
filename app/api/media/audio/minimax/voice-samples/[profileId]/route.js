...
async function serve(request, context, headOnly) {
  const params = await context?.params;
  const profileId = typeof params?.profileId === "string" ? params.profileId.trim() : "";
  const token = new URL(request.url).searchParams.get("token") || "";
  if (!PROFILE_ID_PATTERN.test(profileId) || !TOKEN_PATTERN.test(token)) return notFound();
  const limited = rateLimit(
    `media-minimax-sample:${profileId}:${getClientIP(request)}`,
    SAMPLE_ACCESS_LIMIT,
  );
  if (!limited.success) {
    return Response.json({ success: false, message: "样本访问过于频繁" }, {
      status: 429,
  ...
  const voice = await MinimaxVoice.findOne({
    ...
    sampleTokenExpiresAt: { $gt: new Date() },
  }).select("userId sampleFileId +sampleTokenHash").lean();
  if (!voice || !tokensMatch(token, voice.sampleTokenHash)) return notFound();
  const file = await findOwnedStoredFile({ userId: voice.userId, fileId: voice.sampleFileId });
  if (!file || file.kind !== "voice-sample" || file.ownerType !== "voice-profile" || file.ownerId !== profileId) {
    return notFound();
  }
  try {
    ...
    if (requestedRange && !range) {
      return new Response(null, {
        status: 416,
        headers: { "Cache-Control": "no-store", "Content-Range": `bytes */${size}` },
      });
    }
    ...
    console.error("[MiniMax Audio] serve voice sample:", error);
    return Response.json({ success: false, message: "读取声音样本失败" }, {
      status: 500,
      headers: { "Cache-Control": "no-store" },
    });
  }
