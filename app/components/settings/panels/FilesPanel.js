...
const ownerLabel = type => type === 'library' ? '个人文件' : type === 'project' ? '项目文件' : '对话与生成文件';
...
export default function FilesPanel() {
  ...
  const openDelete = ids => setDialog({ type: 'delete', ids });
  ...
  const saveDialog = () => act(async () => {
    ...
    if (['move', 'copy', 'delete'].includes(type)) {
      for (const id of dialog.ids) {
        ...
        if (type === 'delete') await request(`/api/library/files/${id}`, { method: 'DELETE' });
        setSelected(current => current.filter(value => value !== id));
        if (type === 'delete' || (type === 'move' && dialog.target !== folderId)) setData(current => ({ ...current, files: current.files.filter(file => file.fileId !== id) }));
  ...
  return <div className="space-y-5">
    ...
    <div className="space-y-2" aria-busy={loading || busy}>
      {loading ? <p className="py-8 text-center text-sm text-zinc-500">正在加载文件…</p> : <>
        ...
        {files.map(file => <div key={file.fileId} className={`flex items-center gap-3 rounded-xl border p-3 ${selected.includes(file.fileId) ? 'border-primary/40 bg-primary/5' : 'border-zinc-200 dark:border-zinc-700'}`}>
          ...
          <button disabled={busy} onClick={() => openFile(file)} className="min-w-0 flex-1 text-left"><span className="block truncate text-sm" title={file.name}>{file.name}</span><span className="text-xs text-zinc-500">{sizeLabel(file.size)} · {ownerLabel(file.ownerType)}</span></button>
    ...
    {dialog && <ChatSettingsDialog title={titles[dialog.type]} onClose={closeDialog}>
      ...
      {dialog.type === 'file' ? <fieldset disabled={busy} className="space-y-4">
        ...
        {dialog.type === 'delete' && <p className="text-sm leading-6">确定永久删除所选的 {dialog.ids.length} 个文件？删除后无法恢复。</p>}
        {dialog.type === 'deleteFolder' && <p className="break-all text-sm leading-6">确定永久删除“{dialog.folder.name}”及其中的所有文件和子文件夹？删除后无法恢复。</p>}
        <div className="flex justify-end gap-2"><button type="button" disabled={busy} className={buttonClass} onClick={closeDialog}>取消</button><button disabled={busy || (dialog.name !== undefined && !dialog.name.trim()) || (dialog.type === 'copy' && !dialog.target) || (dialog.type === 'move' && dialog.target === folderId) || (dialog.type === 'moveFolder' && dialog.target === (dialog.folder.parentId || '')) || (dialog.ids && !dialog.ids.length)} className={`rounded-xl px-4 py-2 text-sm text-white disabled:opacity-50 ${dialog.type.startsWith('delete') ? 'bg-red-600' : 'bg-primary'}`}>{busy ? '处理中…' : dialog.type.startsWith('delete') ? '确认删除' : '保存'}</button></div>
      </form>}
    </ChatSettingsDialog>}
  </div>;
}
