...
const newModel={id:'',name:'',providerId:'',upstreamModel:'',group:'自定义',enabled:false,isDefault:false,isTranscriptionDefault:false,sortOrder:0,contextWindow:128000,maxOutputTokens:8192,nativeInputs:['text'],supportsTools:true,supportsWebSearch:true,billingMode:'tokens',pricing:{inputPerMillion:'',outputPerMillion:'',cachedInputPerMillion:'',cacheWritePerMillion:''},requestOptions:{}};
export default function ModelsPanel() {
  ...
  async function save(event) { event.preventDefault();await run(async()=>{
    ...
    const saved=await apiJson(`/api/admin/${tab}${edit.isNew?'':`/${encodeURIComponent(value.id)}`}`,{method:edit.isNew?'POST':'PATCH',body:value});
    setEdit(null);await reload();await loadModels();setNotice(tab==='models'&&!saved.model?.enabled?'模型已停用保存，请完成连接测试后再启用':'设置已保存');
  }); }
  ...
  async function test(item) { await run(async()=>{
    const modelId=tab==='models'?item.id:data.models.find(model=>model.providerId===item.id)?.id;
    if(!modelId)throw new Error('请先为此服务商保存一个模型');
    try {
      ...
      setNotice(`连接测试通过，可启用模型 · ${(result.durationMs/1000).toFixed(1)} 秒 · ${result.text}`);
    } finally {await reload();await loadModels();}
  }); }
  async function enable(item) {await run(async()=>{await apiJson(`/api/admin/models/${encodeURIComponent(item.id)}`,{method:'PATCH',body:{...item,enabled:true}});await reload();await loadModels();setNotice('模型已启用');});}
  const value=edit?.value;
  return <div className="space-y-5">
    ...
    {edit&&<Section title={`${edit.isNew?'添加':'编辑'}${tab==='models'?'模型':'服务商'}`} description={tab==='providers'?'填写服务商提供的接口根地址和密钥。已保存的密钥不会显示。':'先填写价格并保存，再测试连接，测试通过后启用。修改调用参数或服务商连接后需重新测试；价格修改直接用于后续计费。'}><form onSubmit={save} className="space-y-5">
    ...
    {data&&<Section title={tab==='models'?`已配置 ${data.models.length} 个模型`:`已配置 ${data.providers.length} 个服务商`} description={tab==='models'?'按“保存、测试、启用”完成配置。停用模型会保留已有对话记录；连接测试会产生服务商费用。':'连接测试会发送一条简短请求，并产生服务商费用。'}><div className="divide-y divide-zinc-100 dark:divide-zinc-800">{data[tab].map(item=><div key={item.id} className="flex flex-wrap items-center justify-between gap-4 py-4 first:pt-0 last:pb-0"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="text-sm font-medium">{item.name}</h3><span className={`rounded-full px-2 py-0.5 text-[10px] ${item.enabled?'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-400':'bg-zinc-100 text-zinc-500 dark:bg-zinc-800'}`}>{item.enabled?'已启用':'已停用'}</span>{tab==='models'&&<span className="text-xs text-zinc-500">{item.connectionTestedAt?'测试通过':'待连接测试'}</span>}{item.isDefault&&<span className="text-xs text-primary">默认对话</span>}{item.isTranscriptionDefault&&<span className="text-xs text-primary">默认转写</span>}</div><p className="mt-1 break-all text-xs text-zinc-500">{tab==='models'?`${item.group} · ${item.upstreamModel}`:`${protocols.find(protocol=>protocol[0]===item.protocol)?.[1]} · ${item.hasKey?'已设置密钥':'未设置密钥'}`}</p></div><div className="flex gap-2">{tab==='models'&&!item.enabled&&<Action primary onClick={()=>enable(item)} disabled={busy||!item.connectionTestedAt}>启用</Action>}<Action onClick={()=>test(item)} disabled={busy} aria-label={`测试${item.name}`} className="!px-3"><PlugZap size={15}/><span className="hidden sm:inline">测试</span></Action><Action onClick={()=>start(item)} disabled={busy} aria-label={`编辑${item.name}`} className="!px-3"><Pencil size={15}/></Action><Action danger onClick={()=>remove(item)} disabled={busy} aria-label={`删除${item.name}`} className="!px-3"><Trash2 size={15}/></Action></div></div>)}{!data[tab].length&&<p className="py-8 text-center text-sm text-zinc-500">尚未添加{tab==='models'?'模型':'服务商'}</p>}</div></Section>}
  </div>;
}
