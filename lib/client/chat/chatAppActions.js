...
import { getImageModelConfig, validateImageReferences } from "@/lib/media/shared/models";
...
export function useChatAppActions({ toast, messages, loading, model, editingImages, setEditingMsgIndex, setEditingContent, setEditingImages }) {
  ...
  const onEditingImagesSelect = (selectedImages) => {
    ...
    const imageLimit = isImageModel ? getImageModelConfig(model).maxReferenceImages : 1;
    const baseImages = isImageModel ? editingImages : [];
    if (isImageModel) {
      try {
        validateImageReferences(model, [
          ...baseImages.map((image) => ({ name: image.name, type: image.mimeType, size: image.size })),
          ...items.map((image) => image.file),
        ]);
      } catch (error) {
    ...
    const selectedImagesToUpload = isImageModel
      ? items
      : items.slice(0, Math.max(0, imageLimit - baseImages.length));
    const pendingImages = selectedImagesToUpload
      .map((image) => ({
        ...image,
  ...
  const startEdit = (index, msg) => {
    ...
    const existingImages = (Array.isArray(msg?.parts) ? msg.parts : [])
      ...
      .map((part, imageIndex) => ({
        ...
        name: isImageGenerationModel(model) ? part.inlineData.name : `参考图片 ${imageIndex + 1}`,
        size: part.inlineData.size,
        uploadStatus: "ready",
      }));
