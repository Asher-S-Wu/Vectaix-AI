...
import { notifyCreditSummary } from "@/lib/client/credits/events";
...
async function refreshCreditSummary() {
  try {
    ...
    notifyCreditSummary(payload?.credit);
  } catch { /* ignore credit refresh failure */ }
}

function refreshCreditSummaryAfterAbort() {
  void refreshCreditSummary();
  for (const delay of [1500, 5000]) {
    window.setTimeout(() => { void refreshCreditSummary(); }, delay);
  }
}
...
export async function runChat({
  ...
  const requiresCompletedStream = provider === "openai";
  ...
  const payload = {
    ...
    billingOperationId: generateMessageId(),
  };

  const saveConversationMessageTimeline = async (convId, messageId, thinkingTimeline) => {
    if (!convId || !messageId || !Array.isArray(thinkingTimeline) || thinkingTimeline.length === 0) return;
    try {
      await fetch(`/api/conversations/${convId}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ messageId, thinkingTimeline }),
        }
      );
    } catch { }
  };

  const restoreRegenerateMessages = () => {
  ...
  try {
    const mediaGeneration = isMediaGenerationModel(model);
    const res = await fetch(mediaGeneration ? "/api/chat/media" : "/api/chat", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        ...(mediaGeneration ? creditOperationHeaders() : {}),
      },
    ...
    if (!res.ok) {
      ...
      if (res.status === 402 && !errorMessage) errorMessage = "积分余额不足";
    ...
    let streamErrorPersistenceKnown = false;
    let streamErrorMessagePersisted = false;
    let creditReviewNeedsRollback = false;
      ...
      updateThinkingTimeline((prev) => {
        ...
        const next = prev.map((item) => {
          if (
            ...
            || (item?.kind === "image_gen" && item?.status === "running")
          ) {
            changed = true;
    ...
    const applyEventPayload = (payload) => {
      ...
      try {
        ...
        if (["credit_reserved", "credit_settled", "credit_review_required"].includes(data.type)) {
          notifyCreditSummary(data.billing?.credit);
          if (data.type === "credit_review_required" && data.messagePersisted !== true) {
            creditReviewNeedsRollback = true;
          }
          ...
          streamErrorPersistenceKnown = typeof data.messagePersisted === "boolean";
          streamErrorMessagePersisted = data.messagePersisted === true;
        }
      } catch {
        return;
      }
    ...
    if (requiresCompletedStream && !sawDone && streamErrorMessage === null) {
      streamErrorMessage = "回答未完整生成，连接已中断，请重试";
    }

    // 检查流内错误（AI API 在流式传输过程中报错，如上下文超出）
    const streamErrorNeedsRollback = streamErrorMessage !== null && (
      streamErrorPersistenceKnown
        ? !streamErrorMessagePersisted
        : (requiresCompletedStream || fullText.trim() === "")
    );
    if (
      creditReviewNeedsRollback
      || streamErrorNeedsRollback
    ) {
      // 移除本次失败的流式消息
      if (streamMsgId !== null) {
      ...
      throw new Error(streamErrorMessage || "本次模型用量需要人工复核，请稍后查看积分状态");
    }
    ...
    if (!signal?.aborted && sawDone && convIdForSync) {
      (async () => {
        try {
          if (streamMsgId && Array.isArray(thinkingTimeline) && thinkingTimeline.length > 0) {
            await saveConversationMessageTimeline(convIdForSync, streamMsgId, thinkingTimeline);
          }
    ...
    const isAbortError = err?.name === "AbortError" || signal?.aborted;

    if (isAbortError) {
      const restored = restoreRegenerateMessages();
      if (restored) {
        streamMsgId = null;
      } else {
        rollbackPendingTurn();
      }
      refreshCreditSummaryAfterAbort();
    } else {
