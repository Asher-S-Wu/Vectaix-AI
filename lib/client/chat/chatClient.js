...
export async function runChat({
  ...
  const syncConversationMessages = async (convId, nextMessages) => {
    ...
    try {
      ...
      await guestFetch(`/api/conversations/${convId}`,
        {
          method: "PUT",
  ...
  try {
    const res = await guestFetch(isMediaGenerationModel(model) ? "/api/chat/media" : "/api/chat", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const usesContentOnlyModelMessage = model === ABLITERATED_LARGE_MODEL;
    ...
    const ensureSyntheticThoughtRunning = () => {
      if (usesContentOnlyModelMessage) return;
    ...
    if (
      (requiresCompletedStream && streamErrorMessage !== null)
      || (streamErrorMessage && (usesContentOnlyModelMessage || fullText.trim() === ""))
    ) {
      // 移除本次失败的流式消息
      if (streamMsgId !== null) {
    ...
    if (!signal?.aborted && sawDone && convIdForSync) {
      (async () => {
        try {
          const convRes = await guestFetch(`/api/conversations/${convIdForSync}`);
