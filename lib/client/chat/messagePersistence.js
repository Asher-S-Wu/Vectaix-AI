...
const PENDING_MESSAGE_TEXTS = new Set(["正在处理中…"]);
...
function sanitizeProviderState(value, depth = 0) {
  ...
  if (!value || typeof value !== "object") return undefined;
  const next = {};
  if (depth === 0 && Object.hasOwn(value, "responses")) {
    const responses = value.responses;
    if (
      !responses
      || typeof responses !== "object"
      || Array.isArray(responses)
      || !isNonEmptyString(responses.model)
      || !Array.isArray(responses.items)
    ) {
      throw new Error("providerState.responses invalid");
    }
    // Responses 的加密状态必须完整保留，体积由整体请求限制控制。
    next.responses = responses;
  }
  for (const [key, item] of Object.entries(value).slice(0, 100)) {
    if (depth === 0 && key === "responses") continue;
    ...
    const next = parts
      ...
      .map((part) => {
        ...
        if (part.inlineData && typeof part.inlineData === "object") {
          ...
          if (!fileId || !isNonEmptyString(part.inlineData.url)) return Object.keys(entry).length > 0 ? entry : null;
          entry.inlineData = {
            fileId,
...
function hasMeaningfulModelProgress(message, { content, parts, thinkingTimeline, citations, tools } = {}) {
...
function normalizePersistedMessage(message) {
  ...
  const type = ALLOWED_PERSISTED_TYPES.has(message.type) ? message.type : "parts";
  ...
  const parts = isContentOnlyModelText
    ? undefined
    : normalizePersistedParts(message.parts, content);
  if (!isContentOnlyModelText && (!parts || parts.length === 0)) return null;
  const progressParts = Array.isArray(parts) ? parts : [];
  ...
  if (!hasMeaningfulModelProgress(message, { content, parts: progressParts, thinkingTimeline, citations, tools })) {
    const normalizedContent = content.trim();
    const pendingOnly = message.role === "model"
      && (!normalizedContent || PENDING_MESSAGE_TEXTS.has(normalizedContent))
      && progressParts.every((part) => {
        const text = typeof part?.text === "string" ? part.text.trim() : "";
        return !text || PENDING_MESSAGE_TEXTS.has(text);
      });
    if (pendingOnly) {
