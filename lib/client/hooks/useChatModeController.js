...
import { getImageModelOption } from "@/lib/media/shared/models";
...
export function useChatModeController({
  ...
  const requestModelChange = (nextModel) => {
    ...
    const sameImageModel = isImageGenerationModel(model)
      && isImageGenerationModel(nextModel)
      && getImageModelOption(model).id === getImageModelOption(nextModel).id;

    if (messages.length > 0 && model !== nextModel && !sameImageModel) {
      setConfirmModalConfig({
        title: "切换模型",
