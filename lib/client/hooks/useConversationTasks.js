...
export function useConversationTasks({
  ...
  setMessages, setCurrentConversationId, onActivity, onAuthExpired, toast, completionSoundVolume,
}) {
  ...
  return { tasks: tasks.filter(task => task.conversationId === conversationId), submitting, send, stop, deleteMessage };
}
