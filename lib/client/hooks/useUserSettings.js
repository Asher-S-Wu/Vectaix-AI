...
import { DEFAULT_ASSISTANT, DEFAULT_PERMISSIONS } from "@/lib/shared/preferences.mjs";
...
export function useUserSettings() {
  ...
  const [chatMediaSettings, _setChatMediaSettings] = useState({});
  ...
  const fetchSettings = useCallback(async () => {
    try {
      const [data] = await Promise.all([apiJson("/api/settings"), loadModels()]);
      const selected = resolveUsableModelId(readLocalSetting(UI_MODEL_KEY));
      _setModel(selected);
      ...
      _setThemeMode(settings.appearance.themeMode);
      _setFontSize(settings.appearance.fontSize);
      _setCompletionSoundVolume(settings.appearance.completionSoundVolume);
      writeLocalSetting(UI_THEME_MODE_KEY,settings.appearance.themeMode);
      writeLocalSetting(UI_FONT_SIZE_KEY,settings.appearance.fontSize);
      writeLocalSetting(UI_COMPLETION_SOUND_VOLUME_KEY,String(settings.appearance.completionSoundVolume));
      _setChatMediaSettings(settings.chatMediaSettings);
  ...
  const setChatMediaSettings = useCallback(async (nextSettings) => {
    const data = await apiJson("/api/settings", { method: "PUT", body: { chatMediaSettings: nextSettings } });
    _setChatMediaSettings(data.settings.chatMediaSettings);
    return data.settings.chatMediaSettings;
  }, []);

  const resetChatMediaSettings = useCallback(() => _setChatMediaSettings({}), []);
