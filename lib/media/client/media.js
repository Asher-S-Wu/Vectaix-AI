...
function buildDoubaoAudioGenerationFormData(input) {
  const formData = new FormData();
  formData.append("mode", input.mode);
  formData.append("textPrompt", input.textPrompt);
  formData.append("format", input.format);
  formData.append("speechRate", String(input.speechRate));
  formData.append("enableSubtitle", String(input.enableSubtitle));
  for (const reference of input.audioReferences || []) {
    formData.append("referenceAudio", reference.file || reference);
  }
  return formData;
}
...
export async function createDoubaoAudioGeneration(input) {
  const response = await audioFetch("/api/media/audio/doubao/generations", {
    method: "POST",
    body: buildDoubaoAudioGenerationFormData(input),
