import { guestFetch, getGuestLinkId } from "@/lib/client/guestAccess";
...
async function audioFetch(url, options, networkMessage) {
  try {
    return await guestFetch(url, options);
  } catch {
...
export async function generateImage(input) {
  const response = await guestFetch("/api/media/image", {
    method: "POST",
...
export async function editImage({ prompt, size, images }) {
  ...
  const response = await guestFetch("/api/media/image/edit", {
    method: "POST",
    body: formData,
  });
  const data = await readJson(response);
...
function sendAudioSource(file, purpose, { onProgress, signal } = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (guestId) request.setRequestHeader("X-Guest-Link-Id", guestId);
