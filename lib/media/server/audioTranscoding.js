...
const DEFAULT_NORMALIZED_AUDIO_MAX_BYTES = 10 * 1024 * 1024;
...
export async function transcodeAudioClip({
  ...
  const isMinimaxVoiceClone = purpose === AUDIO_UPLOAD_PURPOSES.MINIMAX_VOICE_CLONE;
  const isDoubaoVoiceLibrary = purpose === AUDIO_UPLOAD_PURPOSES.DOUBAO_VOICE_LIBRARY;
  const usesVoiceCloneFormat = isVoiceClone || isMinimaxVoiceClone || isDoubaoVoiceLibrary;
  const sampleRate = usesVoiceCloneFormat ? 24_000 : 44_100;
  const channels = usesVoiceCloneFormat ? 1 : Math.min(2, sourceMetadata.channels);
  const outputLimits = getAudioClipLimits(purpose);
  const maxOutputBytes = outputLimits?.maxOutputBytes || DEFAULT_NORMALIZED_AUDIO_MAX_BYTES;
  const result = await runMediaProcess({
    command: ffmpegPath,
    args: [
      "-nostdin",
    ...
    binaryStdout: true,
    stdoutMaxBytes: maxOutputBytes,
    stdoutOverflowMessage: `转换后的音频仍然超过 ${Math.floor(maxOutputBytes / 1024 / 1024)}MB，请缩短所选片段`,
  });

  const buffer = normalizeStreamedWavHeader(result.stdout);
  if (!buffer.length || buffer.length > maxOutputBytes) {
    throw operationalError(`转换后的音频仍然超过 ${Math.floor(maxOutputBytes / 1024 / 1024)}MB，请缩短所选片段`);
  }
