...
export function requireMediaCreditOperation(request, {
  ...
  return createMediaCreditOperation({ clientOperationId: String(request?.headers?.get?.(CREDIT_OPERATION_HEADER) || "").trim(), userId, feature, fingerprintInput });
}

export function createMediaCreditOperation({ clientOperationId, userId, feature, fingerprintInput }) {
  ...
  if (typeof userId !== "string" || !userId || !FEATURE_PATTERN.test(feature || "")) {
    throw new TypeError("媒体请求标识参数无效");
  }
