...
export function publicImageError(error) {
  const status = error?.status ?? error?.statusCode;
  if (Number.isInteger(status) && status >= 400 && status <= 499 && typeof error?.message === "string") {
    return { message: error.message, status };
  }
  if (status === 503 && error?.code === "SERVICE_NOT_CONFIGURED") {
    return { message: "图片服务尚未配置，请联系管理员", status };
  }
  return { message: "图片处理失败", status: 500 };
}
