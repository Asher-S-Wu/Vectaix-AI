...
export async function storeUpstreamVideoOutput(
  taskId,
  { userId, ownerId, signal, mediaWriteLease } = {},
) {
...
export async function storeUpstreamConversationVideoOutput(
  taskId,
  { userId, conversationId, signal, mediaWriteLease } = {},
) {
  const { input, contentType } = await downloadUpstreamVideoOutput(taskId, { signal });
  return saveConversationVideoBuffer({
    userId,
    conversationId,
    input,
    mimeType: contentType,
