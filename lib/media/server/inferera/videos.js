...
import { saveConversationVideoBuffer, saveVideoBuffer } from "@/lib/media/storage";
...
export async function storeUpstreamVideoOutput(taskId, { userId, ownerId, signal } = {}) {
  const { input, contentType } = await downloadUpstreamVideoOutput(taskId, { signal });
  return saveVideoBuffer({ userId, ownerId, input, mimeType: contentType });
}

export async function storeUpstreamConversationVideoOutput(taskId, { userId, conversationId, signal } = {}) {
  const { input, contentType } = await downloadUpstreamVideoOutput(taskId, { signal });
  return saveConversationVideoBuffer({ userId, conversationId, input, mimeType: contentType });
}

async function downloadUpstreamVideoOutput(taskId, { signal } = {}) {
  ...
  return { input: await response.arrayBuffer(), contentType };
}
