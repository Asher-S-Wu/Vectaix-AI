...
const UPSTREAM_TASK_STATUSES = new Set([
  "queued",
  "running",
  "completed",
  "failed",
...
async function requestMediaKit(path, {
  method = "GET",
  body,
  signal,
  onRequestDispatched,
...
export async function requestMediaKitUploadTicket({ signal } = {}) {
  const data = await requestMediaKit(REQUEST_UPLOAD_PATH, {
    method: "POST",
    body: { tool_name: ENHANCE_VIDEO_TASK_TYPE },
    signal,
  });
...
export async function submitMediaKitVideoEnhancementTask(input, {
  signal,
  onRequestDispatched,
} = {}) {
  ...
  const data = await requestMediaKit(ENHANCE_VIDEO_PATH, {
    method: "POST",
    body,
    signal,
    onRequestDispatched,
...
export async function getMediaKitVideoEnhancementTask(taskId, { signal } = {}) {
  ...
  return Object.freeze({
    ...
    error: status === "failed"
      ? normalizeFailedTaskError(data)
      : status === "canceled"
        ? normalizeVideoEnhancementError({ code: "TASK_CANCELED" })
        : null,
    createdAt: normalizeTimestamp(data.created_at),
