...
import { finalizeMediaKitTaskBilling } from "@/lib/media/server/mediaKit/taskRecords";
...
const MAX_BILLABLE_RESULT_DURATION_SECONDS = 60;
...
async function claimCandidate(candidate, now, { force = false } = {}) {
  ...
  if (candidate.status === "submitting") {
    ...
    if (!force) commonFilter.$and.push(pollDueFilter(now));
  }
  const task = await VideoEnhancementTask.findOneAndUpdate(
    ...
    { new: true },
  ).select("+upstreamTaskId +billingPricingSnapshot +lease.owner").lean();
...
async function failClaimedTask(task, owner, code, { mediaWriteLease, unknown = false } = {}) {
  ...
  await finalizeMediaKitTaskBilling(terminal, {
    status,
    unknown: unknown || code === "INVALID_UPSTREAM_RESPONSE",
    usage: { errorCode: safeError.code },
  }).catch((error) => {
    console.error(
      "[AI MediaKit] finalize terminal billing failed",
...
async function queryAndApplyTask(task, owner, abortController, mediaWriteLease) {
  ...
  if (
    Number.isFinite(upstream.result?.duration)
    && upstream.result.duration > MAX_BILLABLE_RESULT_DURATION_SECONDS
  ) {
    await finalizeMediaKitTaskBilling(task, {
      status: "completed",
      durationSeconds: upstream.result.duration,
      usage: {
        durationSeconds: upstream.result.duration,
        resultRejectedForDurationLimit: true,
      },
    });
  ...
  await finalizeMediaKitTaskBilling(finalizing, {
    status: "completed",
    durationSeconds: upstream.result?.duration,
    usage: { durationSeconds: upstream.result?.duration },
  }).catch((error) => {
    console.error(
      "[AI MediaKit] finalize completed billing failed",
      { taskId: String(task._id), ...safeErrorDetails(error) },
    );
  });
  try {
...
async function reconcileCandidate(candidate, { force = false } = {}) {
  const now = new Date();
  const claimed = await claimCandidate(candidate, now, { force });
  if (!claimed) return;
  ...
  try {
    ...
    if (task.status === "submitting") {
      await failClaimedTask(task, owner, "TASK_FAILED", {
        mediaWriteLease,
        unknown: Boolean(task.submissionDispatchedAt),
      });
      return;
    }
...
export async function syncMediaKitVideoEnhancementTask(task) {
  await dbConnect();
  if (!task || !["submitting", ...ACTIVE_STATUSES].includes(task.status)) return;
  await reconcileCandidate(task, { force: true });
}
...
export async function reconcileMediaKitVideoEnhancementTasks() {
  ...
  const terminalBillingTasks = await VideoEnhancementTask.find({
  ...
  for (const task of terminalBillingTasks) {
    await finalizeMediaKitTaskBilling(task, {
      status: task.status,
      durationSeconds: task.result?.duration,
      unknown: task.error?.code === "INVALID_UPSTREAM_RESPONSE",
      usage: task.result?.duration
        ? { durationSeconds: task.result.duration }
        : { errorCode: task.error?.code || "" },
    }).catch((error) => {
      console.error(
        "[AI MediaKit] reconcile terminal billing failed",
        { taskId: String(task._id), ...safeErrorDetails(error) },
      );
    });
  }
