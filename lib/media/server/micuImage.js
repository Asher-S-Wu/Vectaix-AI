...
async function requestMicuImageOnce({
  model,
  prompt,
  size,
  images = [],
  ...
  const quality = getImageModelConfig(model).fixedQuality;
...
function isHardImageError(error) {
  if (error.name === "AbortError" || error.code === "REQUEST_CANCELLED") return true;
  const detail = `${error.code} ${error.upstreamType || ""} ${error.upstreamMessage || ""}`;
  if (/invalid[ _-]?(api[ _-]?)?key|authentication|permission|access[ _-]?denied|insufficient|quota|billing|balance|credit.*(?:exhaust|insufficient)|moderation|content[ _-]?(?:policy|filter)|safety|invalid[ _-]?(?:parameter|argument)|unsupported|余额|额度|审核|权限|密钥/i.test(detail)) return true;
  return error.status >= 400 && error.status < 500 && ![408, 409, 425, 429].includes(error.status);
}

export async function requestMicuImage(options) {
  // Configuration failures are local and must never enter the retry loop.
  resolveMicuImageConfig();
  getImageModelConfig(options.model);
  const attempts = [];
  for (let attempt = 0; attempt < 5; attempt++) {
    let dispatched = false;
    try {
      const result = await requestMicuImageOnce({
        ...options,
        onRequestDispatched: () => { dispatched = true; options.onRequestDispatched?.(); },
      });
      attempts.push({ success: true, requestId: result.requestId, usage: result.usage });
      return { ...result, attempts };
    } catch (error) {
      attempts.push({ success: false, dispatched, requestId: error.requestId, usage: error.usage, code: error.code, status: error.status, message: error.message, upstreamRejected: error.upstreamRejected === true });
      error.attempts = attempts;
      if (isHardImageError(error) || attempt === 4) throw error;
      try {
        await timers.setTimeout(1000 * 2 ** attempt, undefined, { signal: options.signal });
      } catch {
        throw Object.assign(createAbortError(options.signal?.reason), { attempts });
      }
    }
  }
}
...
export async function generateAndStoreMicuImageFile({
  ...
  const { input, mimeType, requestId, usage, attempts } = await requestMicuImage({
    model, prompt, size, images, signal, onRequestDispatched,
  });
  await onUpstreamComplete?.({ requestId, usage, attempts });
  if (signal?.aborted) throw createAbortError(signal.reason);
