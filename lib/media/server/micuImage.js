...
class MicuImageAgent extends Agent {
  dispatch(options, handler) {
    // Node fetch may supply its own shorter request timeout; keep both phases
...
function createResponseError(response, data) {
  ...
  const code = typeof upstreamCode === "string" ? upstreamCode : "UPSTREAM_ERROR";
  const normalizedCode = code.toLowerCase();
  let message;
  if (response.status === 401 || normalizedCode.includes("invalid_api_key")) {
    ...
    message = "Micu 图片服务不接受当前参数，请检查模型、尺寸、画质和参考图";
  } else {
    message = `Micu 图片服务请求失败（${response.status}）`;
  }
  return createServiceError(message, response.status >= 400 ? response.status : 502, {
    code,
...
export async function requestMicuImage({
  ...
  try {
    ...
    const pending = fetch(images.length ? editEndpoint : endpoint, {
      method: "POST",
      headers,
      body,
    ...
    requestId = readRequestId(data, response);
    if (!response.ok || data?.error || data?.code) throw createResponseError(response, data);
