...
async function callMinimax({ model, input, signal }) {
...
export async function synthesizeMinimaxSpeech(input, { signal } = {}) {
  const voiceSetting = {
  ...
  return {
    ...
    requestId,
    traceId,
    characters: Number(data?.usage?.characters ?? extra?.usage_characters) || input.text.length,
    durationMs: Number(extra?.audio_length) || 0,
  };
}

export async function createMinimaxVoice(input, { signal } = {}) {
  const cloneInput = {
    action: "voice_clone",
    voice_id: input.voiceId,
