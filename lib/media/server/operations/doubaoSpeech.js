...
import { DoubaoAudioError, synthesizeDoubaoSpeech } from "@/lib/media/server/doubaoAudio";
...
function publicMessage(error, fallback) {
  ...
  return message && (errorStatus(error) < 500 || (error instanceof DoubaoAudioError && error.code !== "UPSTREAM_ERROR"))
    ? message
    : fallback;
}
...
export async function generateDoubaoSpeech({ userId: authenticatedUserId, body, clientOperationId, signal }) {
  ...
  try {
    ...
    if (error instanceof CreditError) {
      return creditErrorResult(error, "豆包语音生成费用记录失败");
    }
    return mediaServiceResult(
      {
        success: false,
