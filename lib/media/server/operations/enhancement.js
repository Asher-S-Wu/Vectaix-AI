...
export async function enhanceVideo({ userId: authenticatedUserId, body, clientOperationId, signal }) {
  ...
  try {
    ...
    let input;
    try {
      input = normalizeVideoEnhancementCreateInput(body);
    } catch {
    ...
    if (error instanceof CreditError) {
      return creditErrorResult(error, "视频画质增强费用记录失败");
    }
