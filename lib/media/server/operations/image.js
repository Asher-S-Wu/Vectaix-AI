...
import { createAndStoreImageFiles, finalizeImageBilling, getImageFeature, imageBillingUsage, prepareImageRequest, resolveImageServiceConfig } from "@/lib/media/server/imageService";
...
export async function executeImageOperation({ userId, body, clientOperationId, signal }, { edit = false } = {}) {
  ...
  try {
    ...
    return mediaServiceResult({ success: false, ...(batch ? { results: batch.results } : {}), message: error.message, ...(billing ? { billing } : {}) }, { status: error.status || error.statusCode || 500 });
  } finally {
