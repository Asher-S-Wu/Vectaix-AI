...
function publicMessage(error, fallback) {
  ...
  return message && (errorStatus(error) < 500 || (error instanceof MinimaxAudioError && error.code !== "UPSTREAM_ERROR"))
    ? message
    : fallback;
}
...
export async function generateMinimaxSpeech({ userId: authenticatedUserId, body, clientOperationId, signal }) {
  ...
  try {
    ...
    if (!customVoice) {
      voice = await resolveSystemVoice({ voiceId: input.voiceId, signal: signal });
    }
    ...
    if (error instanceof CreditError) {
      return creditErrorResult(error, "MiniMax 语音生成费用记录失败");
    }
    return mediaServiceResult(
      {
        success: false,
