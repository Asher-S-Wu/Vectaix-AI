...
function getPublicErrorMessage(error, fallback) {
  ...
  return message && (error instanceof QwenAudioError || getErrorStatus(error) < 500)
    ? message
    : fallback;
}
...
export async function generateQwenSpeech({ userId: authenticatedUserId, body, clientOperationId, signal }) {
  ...
  try {
    ...
    try {
      ...
      return creditErrorResult(error, "语音生成费用记录失败");
    }
