...
import { VIDEO_ASPECT_RATIO_OPTIONS, VIDEO_DURATION_OPTIONS, VIDEO_EDIT_RESOLUTION_OPTIONS, VIDEO_IMAGE_ACCEPTED_MIME_TYPES, VIDEO_IMAGE_MAX_BYTES, VIDEO_MODELS, VIDEO_MODES, VIDEO_PROMPT_MAX_LENGTH, VIDEO_RESOLUTION_OPTIONS, VIDEO_SEED_MAX, VIDEO_SOURCE_VIDEO_ACCEPTED_MIME_TYPES, VIDEO_SOURCE_VIDEO_MAX_BYTES, getVideoPromptWeight } from "@/lib/media/shared/models";
...
function jsonMessage(message, status = 400) {
  return mediaServiceResult({ success: false, message }, { status });
}
...
export async function generateVideo({ userId: authenticatedUserId, body, clientOperationId, signal }) {
  ...
  try {
    const user = { userId: authenticatedUserId };
    ...
    try {
      ...
      upstreamTask = await createHappyHorseVideoTask({
        ...
        params: input.params,
        sourceUrls,
        signal: AbortSignal.any([signal, AbortSignal.timeout(TASK_SUBMISSION_TIMEOUT_MS)]),
        onRequestDispatched: () => {
    ...
    if (error instanceof CreditError) {
      return creditErrorResult(error, "视频任务费用记录失败");
    }
