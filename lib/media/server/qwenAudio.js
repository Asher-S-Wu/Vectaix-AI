...
export class QwenAudioError extends Error {
  constructor(message, {
    ...
    this.upstreamRejected = upstreamRejected;
  }
}
...
function createMappedUpstreamError(response, data, operation) {
  ...
  const upstreamRejected = response.status >= 200 && response.status < 500;
...
async function postDashScope(path, body, {
  signal,
  operation,
  onRequestDispatched,
  onUpstreamComplete,
  ...
  if (signal?.aborted) {
    throw signal.reason || Object.assign(new Error("语音请求已取消"), { name: "AbortError" });
  }
  let response;
  try {
  ...
  await onUpstreamComplete?.({
    requestId: String(data?.request_id || response.headers.get("x-request-id") || "").trim(),
  });
  return data;
}

export function isExplicitQwenAudioRejection(error) {
  return error instanceof QwenAudioError && error.upstreamRejected === true;
}
...
export async function synthesizeSpeech(input, {
  ...
  const sampleRate = Number(input?.sampleRate ?? AUDIO_DEFAULT_SAMPLE_RATE);
  ...
  const data = await postDashScope(
    ...
    { signal, operation: "synthesis", onRequestDispatched, onUpstreamComplete }
  );
