...
export async function saveVideoSourceStream({
  ...
  try {
    ...
    if (category === "video") {
      const videoDuration = await probeVideoDuration(getStoredFileAbsolutePath(stored), { signal });
      if (videoDuration < 3 || videoDuration > 60) {
        const error = new Error("输入视频时长必须在 3 至 60 秒之间");
        error.status = 400;
        throw error;
      }
      stored.videoDuration = videoDuration;
      await stored.save();
    }
    return stored;
  } catch (error) {
...
export async function downloadAliyunAudio(url, { signal } = {}) {
  const safeAudioUrl = normalizeAliyunAudioUrl(url);
  if (!safeAudioUrl) {
    throw Object.assign(new Error("语音服务返回了不安全的音频地址"), { status: 502 });
  }
  let response;
  try {
    ...
    if (error?.name === "AbortError") throw error;
    throw Object.assign(new Error("下载试听音频失败"), { status: 502, cause: error });
  }
  if (!response.ok || !response.body) {
    throw Object.assign(new Error("下载试听音频失败"), { status: 502 });
  }
  return response;
}
...
export async function saveAudioFromUrl({
  ...
  if (!["mp3", "wav", "flac"].includes(normalizedFormat)) {
    throw new Error("仅支持保存 MP3、WAV 或 FLAC 音频");
  }
  ...
  const mimeType = normalizedFormat === "wav"
    ? "audio/wav"
