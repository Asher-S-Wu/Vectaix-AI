...
function sanitizeProviderState(value, depth = 0) {
  ...
  if (depth === 0 && Object.hasOwn(value, "responses")) {
    ...
    if (
      ...
      || typeof responses.model !== "string"
      || !responses.model.trim()
...
export function sanitizeThinkingTimeline(value, fieldPath = "thinkingTimeline") {
  if (!Array.isArray(value)) throw new Error(`${fieldPath} must be an array`);
  if (value.length > MAX_TIMELINE_STEPS) {
...
export function sanitizeMessage(message, index, { allowContentFallback = false } = {}) {
  ...
  if (typeof message.model === "string" && message.model.trim()) {
    ...
    if (normalizedMessageModel.length <= MAX_MODEL_CHARS && /^[\w./:-]+$/.test(normalizedMessageModel)) {
  ...
  if (Array.isArray(message.thinkingTimeline) && message.thinkingTimeline.length > 0) {
    const timeline = sanitizeThinkingTimeline(message.thinkingTimeline, `messages[${index}].thinkingTimeline`);
  ...
  for (const part of sourceParts) {
    ...
    if (isPlainObject(part.inlineData)) {
      ...
      nextPart.inlineData = {
        fileId, url, mimeType,
        ...(typeof part.inlineData.name === "string" && part.inlineData.name.trim() ? { name: part.inlineData.name } : {}),
...
export function sanitizeConversationBody(body) {
  ...
  if (body.model !== undefined) {
    ...
    if (!/^[\w./:-]{1,100}$/.test(normalizedModel)) {
      throw new Error("model invalid");
    }
