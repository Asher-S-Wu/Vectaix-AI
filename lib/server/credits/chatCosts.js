...
export function calculateAccumulatedCosts({ model, provider, usageRecords, settings, requestFingerprint }) {
  ...
  const billingMode = settings.rates.chat?.[model]?.billingMode;
  if (!['tokens','upstream-cost'].includes(billingMode)) throw new Error('模型没有配置计费方式');
  let openRouterUsageCost = billingMode === 'tokens' ? null : 0;
  let chatCostUsd = 0;
  for (const record of records) {
    ...
    if (billingMode === 'tokens') {
      ...
      if (
        ...
        throw new Error("模型服务商未返回本轮 usage.cost");
      }
