...
const BILLING_CHAT_RATE_UPGRADES = [
  ...
  {
    previousModelId: "qwen-3.8-max",
    modelId: "qwen-3.8-max-0902",
    fields: ["cachedInputPerMillion"],
  },
  {
    previousModelId: "google/gemini-3.7-flash",
...
async function upgradeStoredBillingSettings() {
  if ((await mongoose.connection.collection('model_registry').findOne({_id:'default'}))?.migratedAt) return false;
  for (let attempt = 0; attempt < 5; attempt += 1) {
    ...
    let changed = Object.hasOwn(existing.rates, "exa");
    for (const { previousModelId, modelId, fields } of BILLING_CHAT_RATE_UPGRADES) {
      if (Object.hasOwn(upgradedChat, previousModelId)) {
        if (Object.hasOwn(upgradedChat, modelId)) {
          throw new Error(`计费设置同时包含 ${previousModelId} 和 ${modelId}，无法迁移`);
        }
        upgradedChat[modelId] = upgradedChat[previousModelId];
        delete upgradedChat[previousModelId];
        changed = true;
      }
...
export async function runCreditMigration() {
  ...
  for (const { previousModelId, modelId } of BILLING_CHAT_RATE_UPGRADES) {
