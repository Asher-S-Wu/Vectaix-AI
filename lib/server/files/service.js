...
export async function listLibrary(userId, folderId) {
  await requireFolder(userId, folderId);
  const [folders, files, context] = await Promise.all([
    FileFolder.find({ userId }).sort({ name: 1 }).lean(),
    StoredFile.find({ userId, folderId: folderId || null }).sort({ originalName: 1 }),
    deletionContext(userId),
  ]);
  return { folders, files: files.map(file => ({ ...fileMetadata(file), deletionBlockedReason: deletionBlockedReason(userId, file, context) })) };
}
...
const managedFileReasons = {
  avatar: '头像文件，请在个人资料或助手设置中更换或移除头像',
  'image-result': '图片生成文件，请在图片生成页面删除对应记录',
  'video-task': '视频生成文件，请在视频生成页面删除对应任务',
  'video-enhancement-task': '视频增强文件，请在视频增强页面删除对应任务',
  'audio-generation': '语音生成文件，请在语音生成页面删除对应记录',
  'voice-profile': '音色文件，请在音色管理页面删除对应音色',
  'audio-processing': '音频处理文件，请在音频处理页面删除对应记录',
};
async function deletionContext(userId) {
  const [refs, tasks] = await Promise.all([
    referencedFileIds(userId),
    WorkbenchTask.find({ userId, status: { $in: ['queued','running','waiting_media','waiting_approval'] } }).select('_id projectId conversationId').lean(),
  ]);
  return { refs, tasks };
}
function deletionBlockedReason(userId, file, { refs, tasks }) {
  if (managedFileReasons[file.ownerType]) return managedFileReasons[file.ownerType];
  if (file.kind === 'audio-source') return '音频源文件，请在对应的音频功能页面删除';
  if (String(file.userId) !== String(userId) || refs.has(file.fileId)) return '文件仍被对话、任务、头像、记忆或技能使用，请先移除对应引用';
  if (tasks.some(task => file.ownerType === 'library' || (file.ownerType === 'project' && String(task.projectId) === file.ownerId) || (file.ownerType === 'conversation' && String(task.conversationId) === file.ownerId) || (file.ownerType === 'task' && String(task._id) === file.ownerId))) return '相关任务正在运行，请结束后再删除';
  return null;
}
export async function assertFilesDeletable(userId, files) {
  const context = await deletionContext(userId);
  const blocked = files.map(file => ({ file, reason: deletionBlockedReason(userId, file, context) })).filter(item => item.reason);
  if (blocked.length) throw workbenchError(`“${blocked[0].file.originalName}”：${blocked[0].reason}${blocked.length > 1 ? `（共 ${blocked.length} 个文件不可删除）` : ''}`, 409);
}
export async function deleteLibraryFiles(userId, ids) {
  if (!Array.isArray(ids) || !ids.length || ids.some(id => typeof id !== 'string')) throw workbenchError('请选择要删除的文件');
  const files = await Promise.all([...new Set(ids)].map(id => requireFile(userId, id)));
  await assertFilesDeletable(userId, files);
  for (const file of files) await deleteStoredFileDocument(file);
}
export async function deleteLibraryFile(userId, id) { await deleteLibraryFiles(userId, [id]); }
