...
export async function getModelConnection(id) {
  ...
  return {model,provider:{id:provider.id,baseUrl:provider.baseUrl,protocol:provider.protocol,apiKey:decryptSecret(provider.encryptedKey,`provider:${provider.id}`)}};
}
...
export async function saveModel(input,{create=false}={}) {
  ...
  await ManagedModel.findOneAndUpdate({id:value.id},{$set:value},{upsert:create,new:true,runValidators:true});
