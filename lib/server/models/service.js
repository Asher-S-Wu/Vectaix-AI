...
export async function getModelConnection(id) {
  ...
  if(!apiKey)throw modelError(`${model.name} 模型服务尚未配置，请联系管理员`,503);
  return {model,provider:{id:config.id,baseUrl:config.baseUrl,protocol:config.protocol,apiKey,headers:{...config.headers,Authorization:`Bearer ${apiKey}`}}};
}
