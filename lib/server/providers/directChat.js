...
import { getModelConnection } from '@/lib/server/models/service';
import { readStoredFileBuffer } from "@/lib/server/storage/service";
import { runGptResponses } from "@/lib/server/providers/gptResponses";
import { runNativeChat } from './nativeProtocols.mjs';
import { publicFetch } from '@/lib/server/integrations/network.mjs';
...
async function toOpenAICompatibleContent(content, role) {
  ...
  for (const part of content) {
    if (part?.type === 'input_audio' && role === 'user') {
      result.push({type:'input_audio',input_audio:part.input_audio});
      continue;
    }
...
function storedChatCompletionMessages(message, model, providerId) {
  const state = message?.providerState?.chatCompletions;
  if (!providerId || state?.providerId !== providerId || state.model !== model || !Array.isArray(state.messages)) return null;
  return state.messages.filter((item) => item && typeof item === "object");
}

async function buildOpenAICompatibleMessages(messages, system, model, providerId) {
  const result = system ? [{ role: "system", content: system }] : [];
  for (const message of messages || []) {
    if (message?.role === "assistant") {
      const storedMessages = storedChatCompletionMessages(message, model, providerId);
...
async function streamOpenAIChatCompletion({
  ...
  if (!["stop", "tool_calls"].includes(finishReason)) {
    throw new Error(finishReason === "length" ? "模型回复达到长度上限，未能完整完成" : "模型回复未完整结束");
  }
  if (![usage?.prompt_tokens, usage?.completion_tokens].every(value => Number.isSafeInteger(value) && value >= 0)) {
...
function chatCompletionsProviderState(model, providerId, messages, persistNativeMessages) {
  if (!persistNativeMessages || !messages.length) return null;
  return { chatCompletions: { model, providerId, messages } };
}
...
export async function runOpenAICompatibleChat({
  ...
  let requestMessages = await buildOpenAICompatibleMessages(messages, system, model, providerId);
  ...
  for (let pass = 0; pass < (hasTools ? (maxToolPasses ?? 11) : 1); pass += 1) {
    if (prepareMessages) requestMessages = await prepareMessages(requestMessages, {protocol:'chat-completions'});
    ...
    const buildRequest = (outputLimit) => ({
      ...
      ...(Number.isSafeInteger(outputLimit) && outputLimit > 0
        ? {
          [outputTokenParameter]: Math.min(
            Number.isSafeInteger(requestExtras?.[outputTokenParameter])
              ? requestExtras[outputTokenParameter]
              : outputLimit,
            outputLimit,
          ),
        }
    ...
    const completion = await streamOpenAIChatCompletion({
      ...
      signal,
      onText: hasTools && !streamToolText ? () => {} : onText,
      onThought,
      onUpstreamRequest,
      onUpstreamId,
      onUsageRecord,
    });
    await onPassComplete?.({ pass, completion });
    ...
    if (!completion.toolCalls.length) {
      if (hasTools && !streamToolText && completion.text) onText(completion.text);
      return {
        text: completion.text,
        thought,
        usage,
        usageRecords,
        providerState: chatCompletionsProviderState(model, providerId, nativeMessages, persistNativeMessages),
      };
    }
...
export async function runDirectChat(options) {
  return runConfiguredChat(options,await getModelConnection(options.model));
}
export async function runConfiguredChat(options,{model:modelConfig,provider}) {
  const fetchImpl=options.fetchImpl ?? publicFetch;
  if (options.tools?.length && !modelConfig.supportsTools) throw new Error('此模型未开启工具调用能力');
  if (provider.protocol === 'responses') return runGptResponses({...options,modelConfig,provider});
  if (['anthropic','gemini'].includes(provider.protocol)) {
    const messages=[];
    for (const message of options.messages) {
      if (!Array.isArray(message.content)) { messages.push(message);continue; }
      const content=[];
      for (const part of message.content) {
        if (part.type!=='private_media') { content.push(part);continue; }
        const buffer=await readStoredFileBuffer(part.media.file);
        content.push({type:'inline_media',inlineData:{mimeType:part.media.file.mimeType,data:buffer.toString('base64')}});
      }
      messages.push({...message,content});
    }
    return runNativeChat({...options,messages,modelConfig,provider,fetchImpl});
  }
  return runOpenAICompatibleChat({...options,providerId:provider.id,client:new OpenAI({apiKey:provider.apiKey,baseURL:provider.baseUrl,defaultHeaders:provider.headers,maxRetries:0,fetch:fetchImpl}),requestModel:modelConfig.upstreamModel,outputTokenParameter:modelConfig.outputTokenParameter,requestExtras:{...modelConfig.requestOptions,[modelConfig.outputTokenParameter]:modelConfig.maxOutputTokens,stream_options:{include_usage:true}},persistNativeMessages:true});
}

export function normalizeProviderError(error) {
  if (error instanceof OpenAI.APIError) {
    const normalized = new Error(`模型请求失败（${error.status || 502}）`);
