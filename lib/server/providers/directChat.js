...
function createVeniceOpenAI() {
  const config = resolveVeniceOpenAIConfig();
  return new OpenAI({
...
function buildOpenAIChatMessages(messages, system) {
...
async function runAIHubMixAuto({ messages, system, tools, getTools, executeTool, signal, onText, onThought }) {
  ...
  let requestMessages = buildOpenAIChatMessages(messages, system);
  let thought = "";
  let usage = null;
  ...
  for (let pass = 0; pass < (hasTools ? 11 : 1); pass += 1) {
    ...
    const completion = await streamOpenAIChatCompletion({
      ...
      request: {
        ...
        ...(activeTools?.length ? { tools: openAIChatTools(activeTools) } : {}),
      },
      signal,
...
function createQwenServiceError(message, status, code) {
  const error = new Error(message);
  error.status = status;
  error.code = code;
  error.isUserFacingQwenError = true;
  return error;
}

function normalizeVeniceQwenError(error) {
  if (error?.isUserFacingQwenError === true) {
    return error;
  }
  if (error?.message?.includes("VENICE_API_KEY")) {
    return createQwenServiceError(
      "Qwen 模型服务尚未配置，请联系管理员",
      500,
      "VENICE_NOT_CONFIGURED"
    );
  }
  ...
  if (status === 402) {
    return createQwenServiceError(
      "Qwen 模型服务余额不足，请联系管理员",
      502,
      "VENICE_BALANCE_INSUFFICIENT"
    );
  }
  if (status === 429) {
    return createQwenServiceError(
      "Qwen 模型服务请求过于频繁，请稍后再试",
      429,
      "VENICE_RATE_LIMITED"
    );
  }
  if (status >= 500) {
    return createQwenServiceError(
      "Qwen 模型服务暂时不可用，请稍后再试",
      502,
      "VENICE_UPSTREAM_FAILED"
    );
  }
  if (error instanceof OpenAI.APIError) {
    return createQwenServiceError(
      status >= 400
        ? "Qwen 模型请求无效，请检查输入内容"
        : "暂时无法连接 Qwen 模型服务，请稍后再试",
      status >= 400 ? status : 502,
      status >= 400 ? "VENICE_REQUEST_FAILED" : "VENICE_CONNECTION_FAILED"
    );
  }
  return error;
}

async function toVeniceQwenContent(content, role) {
  ...
  for (const part of content) {
    ...
    if (!privateMedia || role !== "user") continue;
    if (privateMedia.category === "audio") {
      throw createQwenServiceError(
        "Qwen 3.6 Plus 不支持音频输入",
        400,
        "VENICE_AUDIO_UNSUPPORTED"
      );
    }
    if (privateMedia.category !== "video" || !privateMedia.file) {
      throw createQwenServiceError(
        "Qwen 3.6 Plus 收到了不支持的附件",
        400,
        "VENICE_MEDIA_UNSUPPORTED"
      );
    }

    const mimeType = String(privateMedia.file.mimeType || "").trim().toLowerCase();
    if (!mimeType.startsWith("video/")) {
      throw createQwenServiceError(
        "Qwen 3.6 Plus 的视频格式无效",
        400,
        "VENICE_VIDEO_INVALID"
      );
    }
...
async function buildVeniceQwenMessages(messages, system) {
  const result = system ? [{ role: "system", content: system }] : [];
  let imageCount = 0;

  for (const message of messages || []) {
    const role = message?.role === "assistant" ? "assistant" : "user";
    const content = await toVeniceQwenContent(message?.content, role);
    if (Array.isArray(content)) {
      imageCount += content.filter((part) => part?.type === "image_url").length;
  ...
  if (imageCount > 10) {
    throw createQwenServiceError(
      "Qwen 3.6 Plus 一次最多处理 10 张图片",
      400,
      "VENICE_TOO_MANY_IMAGES"
    );
  }
  return result;
}

async function runVeniceQwen({
  messages,
  system,
  tools,
  getTools,
  ...
  try {
    let requestMessages = await buildVeniceQwenMessages(messages, system);
    const client = createVeniceOpenAI();
    let thought = "";
    let usage = null;
    const hasTools = Boolean(tools?.length);
    ...
    for (let pass = 0; pass < (hasTools ? 11 : 1); pass += 1) {
      ...
      if (completion.toolCalls.length > 1) {
        throw createQwenServiceError(
          "Qwen 3.6 Plus 返回了多个并行工具调用",
          502,
          "VENICE_PARALLEL_TOOL_CALLS"
        );
      }
      const toolCall = completion.toolCalls[0];
      if (!toolCall?.id || !toolCall?.function?.name) {
        throw createQwenServiceError(
          "Qwen 3.6 Plus 返回了无效的工具调用",
          502,
          "VENICE_TOOL_CALL_INVALID"
        );
      }
      const assistantMessage = {
        role: "assistant",
    ...
    throw createQwenServiceError(
      "联网搜索轮次已用完，Qwen 3.6 Plus 未返回最终回答",
      502,
      "VENICE_TOOL_ROUNDS_EXHAUSTED"
    );
  } catch (error) {
    throw normalizeVeniceQwenError(error);
  }
}
...
async function runGemini({ messages, system, tools, getTools, executeTool, signal, onText, onThought }) {
  ...
  for (let pass = 0; pass < 11; pass += 1) {
    ...
    const interaction = await client.interactions.create({
      ...base,
      input,
      ...(activeTools?.length ? { tools: geminiTools(activeTools) } : {}),
    }, { signal });
...
export async function runDirectChat(options) {
  ...
  if (options.model === QWEN_36_PLUS_MODEL) return runVeniceQwen(options);
