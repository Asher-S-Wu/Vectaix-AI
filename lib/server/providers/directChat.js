...
export async function runConfiguredChat(options,{model:modelConfig,provider}) {
  ...
  if (['anthropic','gemini'].includes(provider.protocol)) {
    ...
    return runNativeChat({...options,messages,modelConfig,provider,fetchImpl:publicFetch});
  }
  return runOpenAICompatibleChat({...options,client:new OpenAI({apiKey:provider.apiKey,baseURL:provider.baseUrl,maxRetries:0,fetch:publicFetch}),requestModel:modelConfig.upstreamModel,requestExtras:{...modelConfig.requestOptions,max_completion_tokens:modelConfig.maxOutputTokens,stream_options:{include_usage:true}},persistNativeMessages:true});
}
