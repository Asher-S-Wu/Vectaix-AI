...
async function toOpenRouterContent(content, role) {
...
async function buildOpenRouterMessages(messages, system) {
  const result = system ? [{ role: "system", content: system }] : [];
  for (const message of messages || []) {
    const role = message?.role === "assistant" ? "assistant" : "user";
    result.push({ role, content: await toOpenRouterContent(message?.content, role) });
  }
  return result;
}
...
function openRouterChatRequestExtras(model) {
  if (model === VECTAIX_AUTO_MODEL) return { reasoning_effort: "medium" };
...
async function runOpenRouterChat({ model, messages, system, tools, getTools, executeTool, signal, onText, onThought }) {
  const client = createOpenRouterOpenAI();
  let requestMessages = await buildOpenRouterMessages(messages, system);
  let thought = "";
  let usage = null;
  ...
  for (let pass = 0; pass < (hasTools ? 11 : 1); pass += 1) {
    ...
    const completion = await streamOpenAIChatCompletion({
      ...
      request: {
        model,
        ...openRouterChatRequestExtras(model),
        stream: true,
...
export async function runDirectChat(options) {
  if (
    options.model === VECTAIX_AUTO_MODEL
    || options.model === GEMINI_FLASH_MODEL
    || options.model === QWEN_38_MAX_MODEL
  ) return runOpenRouterChat(options);
  if (
