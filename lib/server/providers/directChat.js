...
const QWEN_38_MAX_0902_UPSTREAM_MODEL = "qwen3.8-max-0902";
const QWEN_WEB_SEARCH_TOOL_NAME = "webSearch";

function createOpenRouterOpenAI() {
  const config = resolveOpenRouterOpenAIConfig();
  return new OpenAI({ apiKey: config.apiKey, baseURL: config.openAIBaseUrl, maxRetries: 0 });
}
...
function buildChatCompletionProfile(model) {
  if (model === QWEN_38_MAX_0902_MODEL) {
    return {
      client: createQwenOpenAI(),
      requestModel: QWEN_38_MAX_0902_UPSTREAM_MODEL,
      requestExtras: {
...
async function runOpenAICompatibleChat({
  ...
  for (let pass = 0; pass < (hasTools ? (maxToolPasses ?? 11) : 1); pass += 1) {
    if (prepareMessages) requestMessages = await prepareMessages(requestMessages);
...
export async function runDirectChat(options) {
  if (options.model === GPT_6_ASTRA_MODEL) return runGptResponses(options);
