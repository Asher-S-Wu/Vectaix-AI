...
import { resolveOpenRouterOpenAIConfig } from "@/lib/modelRoutes";
import { WEB_BROWSING_MAX_ROUNDS } from "@/lib/server/webBrowsing/types";

const GPT_6_ASTRA_UPSTREAM_MODEL = "openai/gpt-6-astra";
const GPT_6_ASTRA_MAX_OUTPUT_TOKENS = 128000;
...
async function runResponses({
  ...
  let input = buildResponseInput(messages, model);
  const nativeItems = [];
  ...
  for (let pass = 0; pass < maxPasses; pass += 1) {
    signal?.throwIfAborted();
    if (prepareMessages) input = await prepareMessages(input);
    ...
    const buildRequest = (outputLimit) => ({
      model: GPT_6_ASTRA_UPSTREAM_MODEL,
      instructions: system,
      input,
      stream: true,
      store: false,
      ...
      max_output_tokens: Number.isSafeInteger(outputLimit) && outputLimit > 0
        ? Math.min(outputLimit, GPT_6_ASTRA_MAX_OUTPUT_TOKENS)
