...
function buildResponseContent(content, role) {
  ...
  if (!Array.isArray(content)) {
    throw gptServiceError("对话内容格式无效", 400);
  }
  if (role === "assistant") {
    return content.map((part) => {
      if (typeof part?.text !== "string") {
        throw gptServiceError("历史回复格式无效", 400);
      }
      return part.text;
    }).join("");
  }
  return content.map((part) => {
    ...
    throw gptServiceError("此模型仅支持文字和图片输入", 400);
  });
}

function buildResponseInput(messages, model, providerId) {
  const input = [];
  for (const message of messages) {
    const role = message.role;
    if (role !== "user" && role !== "assistant") {
      throw gptServiceError("对话消息类型无效", 400);
    }
    const state = message.providerState?.responses;
    if (role === "assistant" && providerId && state?.providerId === providerId && state.model === model) {
      if (!Array.isArray(state.items) || state.items.length === 0) {
        throw gptServiceError("对话上下文无效", 400);
      }
...
async function streamResponse({
    ...
    switch (event.type) {
      ...
      case "response.output_item.done":
        if (!Number.isInteger(event.output_index) || event.output_index < 0 || !event.item?.type) {
          throw gptServiceError("模型返回的回复内容格式无效");
        }
        itemsByIndex.set(event.output_index, event.item);
        break;
      case "response.completed":
        if (event.response?.status !== "completed") {
          throw gptServiceError("模型未能完成这次回复");
        }
        completed = true;
        break;
      case "response.failed":
      case "error":
        throw gptServiceError("模型回复失败，请稍后再试");
      case "response.incomplete":
        throw gptServiceError("模型回复未完成，请缩短问题或对话后再试");
      default:
        break;
    }
  }
  ...
  if (indexedItems.length === 0 || indexedItems.some(([index], position) => index !== position)) {
    throw gptServiceError("模型返回的对话上下文不完整");
  }
  return {
    text,
    thought,
...
async function runResponses({
  ...
  let input = buildResponseInput(messages, model, provider.id);
  const nativeItems = [];
  const hasTools = Boolean(tools?.length);
  const maxPasses = hasTools ? (maxToolPasses ?? WEB_BROWSING_MAX_ROUNDS * 2 + 1) : 1;
  let thought = "";
  let usage = null;
  const usageRecords = [];
  ...
  for (let pass = 0; pass < maxPasses; pass += 1) {
    signal?.throwIfAborted();
    if (prepareMessages) input = await prepareMessages(input, {protocol:'responses'});
    const activeTools = hasTools ? (getTools ? getTools() : tools) : [];
    ...
    const buildRequest = (outputLimit) => ({
      ...
      max_output_tokens: Number.isSafeInteger(outputLimit) && outputLimit > 0
        ? Math.min(outputLimit, modelConfig.maxOutputTokens)
    ...
    if (calls.length === 0) {
      if (!completion.text.trim()) {
        throw gptServiceError("模型没有返回可显示的回复");
      }
      if (hasTools && !streamToolText) onText(completion.text);
      return {
        text: completion.text,
        thought,
        usage,
        usageRecords,
        providerState: { responses: { model, providerId: provider.id, items: nativeItems } },
      };
    }
    ...
    if (calls.length !== 1 || calls.some((call) => (
      ...
      throw gptServiceError("模型返回了无效的联网工具调用");
    }
    if (pass === maxPasses - 1) {
      throw gptServiceError("联网搜索轮次已用完，模型未返回最终回答");
    }
...
export async function runGptResponses(options) {
  try {
    ...
    if (error.status === 401 || error.status === 403) {
      throw gptServiceError("模型服务认证失败，请联系管理员", 500);
    }
    if (error.status === 429) {
      throw gptServiceError("模型请求过于频繁，请稍后再试", 429);
    }
    if (error.status === 400) {
      throw gptServiceError("模型无法处理这次请求，请检查输入内容或对话长度", 400);
    }
    throw gptServiceError("暂时无法连接模型服务，请稍后再试");
  }
}
