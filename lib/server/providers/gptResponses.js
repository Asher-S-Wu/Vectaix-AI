...
async function streamResponse({ client, request, signal, onText, onThought }) {
  ...
  return { text, thought, usage, items: indexedItems.map(([, item]) => item) };
}
  ...
  for (let pass = 0; pass < maxPasses; pass += 1) {
    ...
    const completion = await streamResponse({
      ...
      request: {
        ...
        ...(activeTools.length ? {
          tools: responseTools(activeTools),
          tool_choice: "auto",
