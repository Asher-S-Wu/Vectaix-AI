...
function nativeMessages(messages,model,protocol,providerId) {
  const result=[];
  for (const message of messages) {
    const state=message.providerState?.[protocol];
    if (message.role==='assistant' && providerId && state?.providerId===providerId && state.model===model && Array.isArray(state.messages)) result.push(...state.messages);
...
async function streamAnthropic(response,options) {
  ...
  let usage={},output,responseId,text='',thought='',stopped=false,reason;
  for await (const event of sse(response)) {
    ...
    if (event.type==='message_delta') { usage={...usage,...event.usage};output=event.usage?.output_tokens;reason=event.delta.stop_reason; }
  ...
  if (!usageRecord) throw failure('模型返回的 token 用量不完整');
...
async function streamGemini(response,options) {
  ...
  for await (const event of sse(response)) {
    ...
    if (event.usageMetadata && finishReason) {
...
export async function runNativeChat(options) {
  ...
  let messages=nativeMessages(options.messages,model,protocol,provider.id);
  ...
  for (let pass=0;pass<(tools.length?(options.maxToolPasses??11):1);pass++) {
    ...
    let response;
    try { response=await fetchImpl(url,{method:'POST',headers:{...headers,...provider.headers,'content-type':'application/json'},body:JSON.stringify(buildRequest(limit)),signal,redirect:'error'}); }
    ...
    if (!completion.calls.length) {
      ...
      return {text:completion.text,thought,usage:completion.usage,usageRecords,providerState:{[protocol]:{model,providerId:provider.id,messages:native}}};
    }
