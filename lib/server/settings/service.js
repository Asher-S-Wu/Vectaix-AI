...
export async function updateUserProfileSettings(userId, { avatarFileId, chatSystemPrompt, chatMediaSettings, nickname } = {}) {
  ...
  if (avatarFileId === undefined && normalizedChatSystemPrompt === undefined && normalizedNickname === undefined && chatMediaSettings === undefined) {
  ...
  settings.updatedAt = Date.now();
  await settings.save();
  if (previousAvatar) await deleteStoredFileDocument(previousAvatar);
  return serializeSettings(settings);
}
