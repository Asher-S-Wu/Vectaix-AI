...
import { validateImageOptions } from "@/lib/media/shared/models";
...
const CHAT_MEDIA_SECTIONS = ["image", "audio", "enhancement"];
...
function serializeSettings(settings) {
  ...
  return {
    ...item,
    chatMediaSettings: Object.fromEntries(CHAT_MEDIA_SECTIONS
      .filter(section => item?.chatMediaSettings?.[section] !== undefined)
      .map(section => [section, item.chatMediaSettings[section]])),
...
export async function updateUserProfileSettings(userId, body = {}) {
  ...
  const settings = await ensureSettingsDocument(userId);
  validateChatMediaSettings(chatMediaSettings, settings.chatMediaSettings);
