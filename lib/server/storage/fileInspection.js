...
function matchesImage(buffer, extension) {
  ...
  if (extension === "bmp") {
    return ascii(buffer, 0, 2) === "BM";
  }
  if (extension === "tif" || extension === "tiff") {
    return startsWith(buffer, [0x49, 0x49, 0x2a, 0x00])
      || startsWith(buffer, [0x4d, 0x4d, 0x00, 0x2a])
      || startsWith(buffer, [0x49, 0x49, 0x2b, 0x00])
      || startsWith(buffer, [0x4d, 0x4d, 0x00, 0x2b]);
  }
  return false;
}
...
function matchesVideo(buffer, extension) {
  ...
  if (extension === "mov") {
    return ["ftyp", "moov", "mdat", "free", "wide", "skip"].includes(ascii(buffer, 4, 8));
  }
