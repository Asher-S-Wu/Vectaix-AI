...
export async function bindStoredFiles({ userId, fileIds, ownerType, ownerId }) {
  ...
  const bound = await StoredFile.updateMany(
    { fileId: { $in: normalizedIds }, userId, $or: [{ ownerType: "temporary" }, { ownerType, ownerId: String(ownerId) }] },
    { $set: { ownerType, ownerId: String(ownerId) } }
  );
  if (bound.matchedCount !== normalizedIds.length) throw new Error("附件已被其他内容占用");
  return files;
}
...
export async function deleteStoredFileDocument(file) {
  ...
  await WorkspaceDocument.deleteOne({ userId: file.userId, fileId: file.fileId });
