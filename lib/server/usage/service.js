...
export async function currentMonthUsage(userId) {
  const { month, createdAt } = dateFilter(new URLSearchParams());
  const [totals] = await CreditTransaction.aggregate([
    { $match: { userId: new mongoose.Types.ObjectId(userId), type: 'model_usage', status: { $in: ['settled', 'review_required'] }, createdAt } },
    { $group: { _id: null, ...costTotals() } },
    { $project: { _id: 0 } },
  ]);
  return { ...emptyTotals(), ...totals, month };
}
