...
const TERMINAL_ENHANCEMENT_STATUSES = ['completed', 'failed', 'canceled'];
...
export async function deleteUserAndData(id) {
    ...
    try {
        ...
        const activeEnhancementTasks = await VideoEnhancementTask.countDocuments({
            userId,
            $or: [
                { status: { $nin: TERMINAL_ENHANCEMENT_STATUSES } },
                { 'billing.status': { $in: ['reserved', 'settling'] } },
            ],
        });
        if (activeEnhancementTasks > 0) {
            await cancelUserDeletionFence(deletionLease);
            deletionLease = null;
            throw new CreditError('该用户仍有运行中的视频画质增强任务，请等待任务完成后再删除', {
                code: 'USER_ACTIVE_ENHANCEMENT_TASKS',
                statusCode: 409,
            });
        }
