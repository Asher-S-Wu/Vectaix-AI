...
import {calculateChatCost,createPricingSnapshot,getChatReservationPoints} from '@/lib/server/credits/pricing';
...
import {InsufficientCreditsError} from '@/lib/server/credits/errors';
...
export async function transcribeRecording({userId,recording,clientOperationId,signal},{runModel=runDirectChat}={}) {
  ...
  const estimate=calculateChatCost({model:model.id,inputTokens,outputTokens},settings);
  const credit=await getCreditSummary(userId);
  const limit=getChatReservationPoints(settings);
  if(estimate.points>limit || (!credit.unlimited&&estimate.points>credit.availablePoints))throw new InsufficientCreditsError({required:estimate.points,available:Math.min(limit,credit.availablePoints)});
  const reservation=await reserveCredits({operationId:operation.operationId,userId,points:estimate.points,feature:'voice_transcription',provider:model.provider,model:model.id,usage:{requestFingerprint:operation.requestFingerprint,durationSeconds:recording.durationSeconds},pricingSnapshot:snapshot,executionClaimId:operation.executionClaimId});
  ...
  async function settle() {
    ...
    const transaction=await settleCredits({operationId:operation.operationId,...costs,usage:{...costs.usage,durationSeconds:recording.durationSeconds},pricingSnapshot:snapshot,upstreamRequestIds:ids,allowAdditionalDebit:false});
    finalized=transaction.status==='settled';
    if(!finalized)throw Object.assign(new Error('转写用量需要核查，请稍后查看积分记录'),{status:409});
    return transaction;
  }
  try {
    ...
    return {text:result.text,billing:billingResult(transaction,await getCreditSummary(userId))};
  } catch(error) {
    if(!finalized) {
