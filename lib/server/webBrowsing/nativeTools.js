...
export function getWebToolDefinitions(apiNames) {
  return buildSharedToolDefinitions(apiNames);
}
...
export async function executeWebBrowsingNativeToolCall({
  ...
  if (apiName === WebBrowsingApiName.search) {
    ...
    const result = await runtime.search(args, { signal, onExaRequestState });
    if (result.success) {
  ...
  if (apiName === WebBrowsingApiName.crawlSinglePage || apiName === WebBrowsingApiName.crawlMultiPages) {
    ...
    const result = apiName === WebBrowsingApiName.crawlSinglePage
      ? await runtime.crawlSinglePage(args, { signal, onExaRequestState })
      : await runtime.crawlMultiPages(args, { signal, onExaRequestState });
    if (result.success) {
