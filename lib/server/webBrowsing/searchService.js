import { tinyfishFetch, tinyfishSearch } from "@/lib/server/search/providers/tinyfish";
import {
...
function errorInfo(error, fallbackMessage) {
  ...
  return {
    name: error instanceof Error ? error.name : "TinyFishError",
...
export class SearchService {
  ...
  async webSearch({ query, language }, options = {}) {
    try {
      const startedAt = Date.now();
      const data = await tinyfishSearch({ query, language }, {
