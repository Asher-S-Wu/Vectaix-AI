...
export async function ensureBuiltinSkills(userId) {
  if (await UserSettings.exists({ userId, skillsInitialized: true })) return;
  await WorkbenchSkill.bulkWrite(BUILTIN_SKILLS.map((skill) => ({ updateOne: {
    filter: { userId, builtinKey: skill.builtinKey },
    update: { $setOnInsert: { ...skill, content: `---\nname: ${skill.name}\ndescription: ${skill.description}\n---\n${skill.content}`, userId, enabled: true } },
    upsert: true,
  } })));
  await UserSettings.updateOne({ userId }, { $set: { skillsInitialized: true } }, { upsert: true });
}

export async function listEnabledSkills(userId, { conversationId } = {}) {
  await ensureBuiltinSkills(userId);
  const capabilities = conversationId ? await getConversationCapabilities(userId, conversationId) : null;
  return WorkbenchSkill.find({ userId, enabled: true, ...(capabilities ? { _id: { $nin: capabilities.disabledSkillIds } } : {}) }).sort({ createdAt: 1 }).lean();
}
...
export async function getMemoryContext(userId, projectId = null, { conversationId } = {}) {
  if (conversationId && !(await getConversationCapabilities(userId, conversationId)).memoryEnabled) return '';
  const scopes = [];
...
export async function saveMemory({ userId, projectId = null, content, source = "manual", conversationId = null }) {
  const value = textField(content, "记忆内容", 4000, true);
  if (!["manual", "automatic"].includes(source)) throw workbenchError("记忆来源无效");
  if (conversationId && !(await getConversationCapabilities(userId, conversationId)).memoryEnabled && source === 'automatic') return null;
  if (projectId !== null) {
    const project = await requireProject(userId, String(projectId));
  ...
  return WorkbenchMemory.create({ userId, projectId, scope: projectId ? "project" : "personal", content: value, source, conversationId });
}
