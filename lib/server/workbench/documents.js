...
export async function createScopedFileQuery({ userId, projectId = null, conversationId = null }) {
  ...
  const scopes = [{ ownerType: 'library' }];
  const taskScopes = [];
...
export async function deleteConversationDocument({ userId, conversationId, projectId, fileId }) {
  return withProjectLock(userId, "task-creation", async () => {
    ...
    if (await WorkbenchTask.exists({ userId, conversationId, status: { $in: ["queued", "running", "waiting_media", "waiting_approval"] } })) throw workbenchError("请等待当前任务结束后再删除资料", 409);
...
async function deleteLockedProjectDocument({ userId, projectId, fileId }) {
  ...
  if (await WorkbenchTask.exists({ userId, projectId, status: { $in: ["queued", "running", "waiting_media", "waiting_approval"] } })) throw workbenchError("请等待项目任务结束后再删除资料", 409);
...
export async function indexStoredDocument({ userId, file, projectId = null, conversationId = null, input }) {
  if (String(file.userId) !== String(userId)) throw workbenchError('文件不存在', 404);
  if (!Object.hasOwn(TYPES, file.extension)) throw workbenchError('此文件格式不支持文档索引');
  const buffer = input || await (await import('node:fs/promises')).readFile(getStoredFileAbsolutePath(file));
  const indexed = await extractDocument(buffer, file.extension);
  await WorkspaceDocument.updateOne({ userId, fileId: file.fileId }, { $set: { projectId, conversationId, chunks: indexed.chunks, tables: indexed.tables } }, { upsert: true, runValidators: true });
  return indexed;
}
