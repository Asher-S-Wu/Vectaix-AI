...
async function accountBillingLocked(task, operationId) {
  ...
  const charged = Number(transaction.charged);
  if (!Number.isFinite(charged)) throw new Error('媒体计费记录无效');
  const previous = { $sum: { $map: { input: '$mediaTasks', as: 'media', in: { $cond: [{ $eq: ['$$media.operationId', operationId] }, '$$media.accountedPoints', 0] } } } };
  const updated = await WorkbenchTask.updateOne({
    _id: task._id,
    mediaTasks: { $elemMatch: { operationId, $or: [{ accountedPoints: { $ne: charged } }, { billingStatus: { $ne: transaction.status } }] } },
  }, [{ $set: {
    chargedPoints: { $add: ['$chargedPoints', { $subtract: [charged, previous] }] },
    mediaTasks: { $map: { input: '$mediaTasks', as: 'media', in: { $cond: [
      { $eq: ['$$media.operationId', operationId] },
      { $mergeObjects: ['$$media', { accountedPoints: charged, billingStatus: transaction.status }] },
      '$$media',
    ] } } },
  } }]);
  if (updated.modifiedCount) await appendTaskEvent(task, 'billing', '媒体计费状态已更新', { operationId, chargedPoints: charged, status: transaction.status });
}

export async function syncTaskMediaBilling(task) {
  const current = await WorkbenchTask.findOne({ _id: task._id, userId: task.userId }).select('mediaTasks');
  if (!current) return;
  for (const media of current.mediaTasks) {
    ...
    const record = await Model.findOne({ userId: task.userId, 'billing.operationId': media.operationId }).select('_id status videoFileId error').lean();
    if (!record) continue;
...
export async function registerMediaTools({ registry, task, signal, assertActive }) {
  const userId = String(task.userId), taskId = String(task._id), projectId = String(task.projectId);
  ...
  async function projectMediaQuery() {
    const projectTasks = await WorkbenchTask.find({ userId, projectId }).select('_id').lean();
    return { userId, $or: [{ ownerType: 'project', ownerId: projectId }, { ownerType: 'task', ownerId: { $in: projectTasks.map((item) => String(item._id)) } }] };
  }
  registry.add(defineTaskTool('list_media', '列出当前项目及项目任务已经生成的图片、音频与视频素材，使用返回的文件编号继续编辑。', {}, async () => {
  ...
  async function ownedFile(fileId, category) {
    ...
    if (!file) throw new Error('素材不属于当前项目或类型不符');
    return file;
  }
  ...
  async function artifact(fileId, name, operationId) {
    ...
    const claim = await WorkbenchTask.updateOne({ _id: task._id, mediaTasks: { $elemMatch: { operationId, artifactClaimed: false } } }, { $set: { 'mediaTasks.$.artifactClaimed': true } });
    if (claim.modifiedCount !== 1) throw new Error('该媒体结果已经登记，请勿重复执行');
  ...
  async function run({ kind, feature, service, body, name, callId }) {
    ...
    const claimed = await WorkbenchTask.updateOne({ _id: task._id, stopRequested: false, status: 'running', mediaTasks: { $not: { $elemMatch: { callId } } } }, { $push: { mediaTasks: { callId, operationId, kind, status: 'submitting', name, accountedPoints: 0, artifactClaimed: false, createdAt: new Date() } } });
