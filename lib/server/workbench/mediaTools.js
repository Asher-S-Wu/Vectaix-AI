...
export async function syncTaskMediaBilling(task) {
  ...
  for (const media of current.mediaTasks) {
    ...
    if (record && !['queued','running','waiting_media'].includes(current.status) && !TERMINAL.has(record.status)) {
    ...
    await accountBilling(task, media.operationId);
    if (!['queued','running','waiting_media'].includes(current.status) && record.status === 'completed' && record.videoFileId && !media.artifactClaimed) {
