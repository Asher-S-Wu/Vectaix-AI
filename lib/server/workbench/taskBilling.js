...
export async function createTaskBilling(task, signal) {
  const settings = await getBillingSettings();
  const provider = getModelConfig(task.model).provider;
  ...
  const api = {
    async resolveMaxOutputTokens({ pass, inputPayload }) {
      ...
      const inputTokens = estimateChatInputTokens({ inputPayload, provider });
      ...
      let outputTokens = WORKBENCH_LIMITS.maxOutputTokens;
      while (outputTokens >= 512 && cost(outputTokens) > available) outputTokens = Math.floor(outputTokens / 2);
      if (outputTokens < 512) throw new Error('积分不足，无法继续执行任务');
