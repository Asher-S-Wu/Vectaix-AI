...
async function createTaskUnlocked(userId, body) {
  ...
  if (parentTaskId && !await WorkbenchTask.exists({ _id: parentTaskId, userId, conversationId: conversation._id, status: { $nin: ['queued','running','waiting_media'] } })) throw workbenchError('只能继续本对话中已结束的任务');
