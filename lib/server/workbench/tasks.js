...
export async function ownedTask(userId,id) {
  ...
  if (operationIds.length) {
    const transactions = await CreditTransaction.find({ userId, operationId: { $in: operationIds } }).select('operationId status actualCostCny').lean();
    ...
    if (modelTransaction && ['settled', 'released', 'rejected'].includes(modelTransaction.status)) {
      await WorkbenchTask.updateOne({ _id: id, userId, activeOperationId: task.activeOperationId }, { $inc: { costCny: modelTransaction.status === 'settled' ? modelTransaction.actualCostCny : 0 }, $set: { activeOperationId: null } });
    }
