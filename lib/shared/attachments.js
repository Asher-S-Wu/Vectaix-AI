...
export const DOCUMENT_EXTENSIONS = Object.freeze(["pdf", "docx", "xlsx", "csv", "txt", "md"]);

const VIDEO_EXTENSIONS = ["mp4", "mov", "webm", "m4v"];
const AUDIO_EXTENSIONS = ["mp3", "wav", "m4a", "aac", "ogg", "weba"];
const EXTRA_IMAGE_EXTENSIONS = ["bmp", "tif", "tiff"];
...
export const ATTACHMENT_LIMITS = Object.freeze({
  ...
  document: { maxBytes: MAX_FILE_BYTES },
});
...
export function getAttachmentCategory({ extension, mimeType }) {
  ...
  if (
    isImageMimeType(normalizedMime)
    || ["jpg", "jpeg", "png", "gif", "webp", ...EXTRA_IMAGE_EXTENSIONS].includes(ext)
  ) {
    return "image";
  }
...
export function getAttachmentInputType(category) {
  ...
  if (normalizedCategory === "document") return "document";
  return "";
}
...
export function getAttachmentAcceptForModel({ supportsImages, supportsVideo = false, supportsAudio = false, supportsDocuments = false }) {
  const accept = [];
  if (supportsDocuments) accept.push(...DOCUMENT_EXTENSIONS.map(extension => `.${extension}`));
