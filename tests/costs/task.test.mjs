...
test('高单价模型不限制普通用户输出，逐轮累加实际花费并保留上下文限制', async () => {
  ...
  assert.ok(Math.abs(stored.costCny - 0.7392) < 1e-12);
  ...
  assert.ok(Math.abs(first.actualCostUsd - 0.11) < 1e-12);
  assert.ok(Math.abs(first.actualCostCny - 0.7392) < 1e-12);
  ...
  assert.ok(Math.abs(stored.costCny - 1.1088) < 1e-12);
