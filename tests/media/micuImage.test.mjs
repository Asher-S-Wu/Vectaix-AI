...
test("正常大小的图片 base64 不会因编码验证导致栈溢出", async (t) => {
  ...
  PNG.copy(input);
  t.mock.method(globalThis, "fetch", async () => Response.json({ data: [{ b64_json: input.toString("base64") }] }));
  const result = await requestMicuImage(PARAMS);
...
test("服务未配置时不发送任何网络请求", async (t) => {
  const { requestMicuImage } = await requestModule(t);
  delete process.env.MICU_API_KEY;
  const fetchMock = t.mock.method(globalThis, "fetch", async () => imageResponse());
...
for (const [label, body] of [
  ...
  test(`${label}时明确报错，不下载 URL 或猜测结果`, async (t) => {
    const { requestMicuImage } = await requestModule(t);
    const fetchMock = t.mock.method(globalThis, "fetch", async () => Response.json({ ...body, request_id: "bad-result" }));
    await assert.rejects(requestMicuImage(PARAMS), (error) => {
...
test("非 JSON 的 4xx 响应仍保留明确拒绝状态", async (t) => {
  const { requestMicuImage } = await requestModule(t);
  t.mock.method(globalThis, "fetch", async () => new Response("Access denied", {
...
test("已取消请求不发送，进行中的请求会取消", async (t) => {
  const { requestMicuImage } = await requestModule(t);
  const fetchMock = t.mock.method(globalThis, "fetch", (_url, { signal }) => rejectOnAbort(signal));
...
test("保存前等待完成回调，回调失败时不保存图片", async (t) => {
  ...
  t.mock.method(globalThis, "fetch", async () => imageResponse({ request_id: "completed-id", usage }));
