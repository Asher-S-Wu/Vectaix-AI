...
const settings={version:1,pricingDate:'2026-09-10',usdToCny:1,rates:{chat:{custom:{billingMode:'tokens',inputPerMillion:2,cachedInputPerMillion:1,cacheWritePerMillion:3,outputPerMillion:6},router:{billingMode:'upstream-cost',inputPerMillion:1,outputPerMillion:1}}}};
...
test('上游实际费用模式严格要求 cost，缺失时不切换计费方法',()=>{
  ...
  assert.equal(result.actualCostCny,0.12);
});
